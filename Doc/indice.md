...
| `app/productos/page.tsx` | `/productos` | CRUD de productos: alta, edición, eliminación, subida de imagen WebP a Supabase Storage. Vista grid/lista, filtro categoría, búsqueda, sort, paginación (12/pág). Usa la VIEW `productos_con_estado`. Botón **"🔁 Transferir"** (28/07): modo selección con checkboxes (solo productos `origen != 'proveedor'`), barra flotante para elegir proveedor destino y confirmar — crea filas pendientes en `transferencias_productos`. |
...
| `app/envio-nube/page.tsx` | `/envio-nube` | Gestión de envíos: asignar paquetería/guía a ventas pagadas, cambiar estado (Pendiente→En tránsito→Entregado/Cancelado). Pestaña **"Paquetes por proveedor"** (28/07): filtro de proveedores + tabla con alto/ancho/peso registrados por cada uno (o aviso si falta), leyendo `paquetes_envio` por `venta_item`. |
...
| `app/proveedores/page.tsx` | `/proveedores` | Portal de proveedores — **protegido por sesión + rol `proveedor`** (desde 23/07/2026; antes era público con solo email). Pestañas: "Administración" (pago + seguimiento de envío por producto vendido + **registro de medidas/peso del paquete**, editable inline); registrar producto; mis solicitudes; mis productos (con **toggle grid/lista**, 28/07); **Transferencias** (aceptar/rechazar productos que el admin quiere transferir, 28/07); mensajes; ajustes. |
...
| `transferencias.ts` | `Transferencia`, `crearTransferencias()`, `fetchTransferenciasPendientes()`, `aceptarTransferencia()`, `rechazarTransferencia()` | Transferencia de productos admin→proveedor con doble confirmación. `aceptarTransferencia`/`rechazarTransferencia` llaman RPCs `security definer` (`aceptar_transferencia`/`rechazar_transferencia`) que solo el proveedor destino puede ejecutar. Nuevo 28/07/2026. |
| `paquetes.ts` | `PaqueteEnvio`, `fetchPaquetesPorVentaItems()`, `guardarPaquete()` | Medidas (alto/ancho) y peso del paquete, registrados por el proveedor por línea de venta (`venta_item_id`). Usado en `proveedores` (captura) y `envio-nube` (lectura con filtro por proveedor). Nuevo 28/07/2026. |
...
| `seccion-28-07-2026.md` | 28/07/2026 | **Transferencia de productos** admin→proveedor con doble confirmación (`lib/transferencias.ts`), fix de bug del importador CSV que marcaba `origen='proveedor'` sin dueño real, **registro de medidas/peso de paquete** por línea de venta (`lib/paquetes.ts`) visible en Proveedores y Envíos (con filtro), **fix de bug real de recursión infinita en RLS** entre `ventas`/`venta_items`, toggle grid/lista en "Mis productos" del proveedor, **seguimiento de pedido propio** en `/mi-cuenta` (línea de tiempo + mapa OpenStreetMap embebido) reemplazando links externos a paqueterías. |
