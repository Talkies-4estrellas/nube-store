...
- [ ] **Confirmar que se corrió en Supabase todo el SQL de la sesión del 23/07**: trigger `handle_new_user`, policies RLS "propio"/"proveedor" de `clientes`/`ventas`/`venta_items`/`envios`, función `actualizar_mi_perfil`, tabla `config_pagos_secretos`, y las ~18 columnas nuevas de `productos` para el histórico del Excel 2023 — se le fue pasando por bloques, no hay confirmación de que todo quedó aplicado
