...
export default function QuienesSomosPage() {
  ...
  useEffect(() => {
    supabase.from('config_storefront').select('quienes_somos').eq('id', 1).single()
      .then(({ data }) => setTexto(data?.quienes_somos ?? ''))
  }, [])
    ...
    <div style={{ minHeight: '100vh', background: '#f0f2f8', padding: '32px 20px 60px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '36px 40px', boxShadow: '0 4px 24px rgba(37,40,85,0.10)' }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: NAVY, marginBottom: 20 }}>Quienes somos</h1>
          ...
          {texto === null ? (
            ...
            <p style={{ fontSize: 14, color: '#9ca3af' }}>Todavía no hay contenido publicado.</p>
          )}
        </div>
      </div>
    </div>
